...
import { LogOut, Menu, ChevronLeft, ChevronRight, LayoutDashboard, FolderOpen, FileText, Lock, Palette, Search, Moon, Sun, ChevronDown, LayoutGrid, X, User, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Checkbox } from "@/components/ui/checkbox";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { userApi, roleApi, projectApi } from "@/services/api";
  ...
  const getFilteredUsers = (): any[] => {
    ...
    if (userSearch) {
      filtered = filtered.filter(
        (user: any) =>
          (user.username || user.name || "").toLowerCase().includes(userSearch.toLowerCase()) ||
    ...
    <div className={`min-h-screen ${darkMode ? "bg-[#DAE0F1]" : "bg-gray-100"}`}>
      ...
      >
        ...
        <nav className={`flex flex-col gap-3 ${sidebarOpen ? "px-7 py-10" : "px-3 py-10"}`}>
          {/* Admin Dashboard */}
          <button onClick={() => navigate("/admin")} className={`flex items-center gap-3 rounded-lg px-4 py-3 text-white bg-[#2d3a5a] transition w-full ${!sidebarOpen ? "justify-center" : ""}`} title="Admin Dashboard">
        ...
        {currentStep === 1 && (
          <div className="px-8 pb-12">
            ...
            <div className="flex justify-between mt-8">
              ...
              <button
                onClick={handleNext}
                disabled={!formData.projectId || !formData.systemApplicationName || (!formData.selectedMethodologies.csv && !formData.selectedMethodologies.csa)}
                ...
                <div className="divide-y divide-gray-300">
                  {getFilteredUsers().map((user) => (
                    <div key={user.id} className="flex items-center hover:bg-gray-50 transition px-4 py-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-gray-900">{user.username || user.name}</p>
                      </div>
                      <div className="flex-1">
        ...
        {currentStep === 4 && (
          <div className="px-8 pb-12">
                ...
                {Array.from(selectedUsers).map((userId: any) => {
                  ...
                  return (
                    <div key={userId_num} className="flex border-b border-[#A9A4A0] hover:bg-gray-50">
                      <div className="flex-1 px-6 py-4 text-sm text-gray-900">{user.username || user.name}</div>
            ...
            <div className="flex justify-between mt-8">
              <button
                onClick={handlePrevious}
                className="px-6 py-2 text-gray-700 border border-gray-400 rounded-full hover:bg-gray-50 font-semibold disabled:opacity-50"
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    // Preparation of team members payload
                    const teamMembers = Array.from(selectedUsers).map(userId => {
                      const roleName = userRoles[userId as number] || "viewer";
                      const roleObj = allRoles.find(r => r.name.toLowerCase() === roleName.toLowerCase());
                      return {
                        user_id: userId as number,
                        role_id: roleObj ? roleObj.id : 2 // Fallback to viewer role ID or similar
                      };
                    });

                    const payload = {
                      name: formData.systemApplicationName || `Project-${formData.projectId}`,
                      description: formData.description,
                      change_number: formData.changeNumber,
                      system_application_name: formData.systemApplicationName,
                      methodologies: formData.selectedMethodologies,
                      gamp_categories: formData.gampCategories,
                      regulations: formData.regulations,
                      csv_deliverables: formData.csvDeliverables,
                      csa_deliverables: formData.csaDeliverables,
                    ...
                    // ── Auto-create documents for checked deliverables ──────────
                    if (projectId) {
                      const { documentApi } = await import("@/services/api");

                      // Map CSV deliverable keys to human-readable filenames
                      const csvDocMap: Record<string, string> = {
                        urs:     "URS - User Requirements Specification",
                        gap:     "GxP Assessment",
                        crf:     "CFR Part 11 (ERES) Assessment",
                        srs:     "SRS – System Risk Assessment",
                        valPlan: "Validation Plan",
                        frs:     "FRS – Functional Requirement Specification",
                        fra:     "FRA – Functional Risk Assessment",
                      ...
                      // Map CSA deliverable keys to filenames
                      const csaDocMap: Record<string, string> = {
                        ubs:          "URS - User Requirements Specification",
                        urra:         "URRA – User Requirement Risk Assessment",
                        gap:          "GxP Assessment",
                        crf:          "CFR Part 11 (ERES) Assessment",
                        srs:          "SRS – System Risk Assessment",
                        valPlan:      "Validation Plan",
                        ...
                        fra:          "FRA – Functional Risk Assessment",
                        ds:           "DS – Design Specification",
                        iqScripted:   "IQ Scripted Test Script",
                        iqUnscripted: "IQ Unscripted Test Script",
                        oqScripted:   "OQ Scripted Test Script",
                        oqUnscripted: "OQ Unscripted Test Script",
                        pqScripted:   "PQ Scripted Test Script",
                        pqUnscripted: "PQ Unscripted Test Script",
                        rtm:          "RTM – Requirement Traceability Matrix",
                        vsr:          "Validation Summary Report",
                      };
                      ...
                      await Promise.allSettled(
                        Array.from(docsToCreate).map(docName =>
                          documentApi.create(docName, projectId).catch(e =>
                            console.warn(`Could not create doc "${docName}":`, e)
                          )
                        )
                      );
                    ...
                    alert("Error creating project. Please check if project name is unique.");
                  } finally {
                    setIsSubmitting(false);
                  }
                }}
                className="px-8 py-2 bg-[#11172B] text-white rounded-full hover:opacity-90 font-semibold flex items-center gap-2"
              >
                {isSubmitting ? (
                  <>
