...
import { ChevronLeft, ChevronRight, LogOut, Moon, Sun, LayoutDashboard, FolderOpen, FileText, Lock, Search, MoreVertical, ChevronDown, Trash2, User, ClipboardList } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { userApi, clearAuthSession } from "@/services/api";
...
const ManageUser = () => {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const apiUsers = (data.users || []).map((u: any) => ({
        ...
        createdDate: u.created_at ? new Date(u.created_at).toLocaleDateString() : "-",
        lastLoginDate: u.last_login ? new Date(u.last_login).toLocaleDateString() : "-",
        role: u.role,
        role_id: u.role_id || (u.role?.id ?? undefined),
        role_name: u.role?.name || undefined,
        project_allocations: u.project_allocations || [],
      }));
      setUsers(apiUsers);
    } catch (err: any) {
  ...
  const handleViewEdit = (user: UserItem) => {
    navigate("/edit-user", { state: { user } });
    setOpenMenuId(null);
  };

  const handleDisableClick = (user: UserItem) => {
        ...
        <main className="p-8 pt-24">
          ...
          {error && (
            <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 mb-6 flex items-center justify-between">
              ...
              <button onClick={loadUsers} className="text-sm text-amber-900 font-semibold hover:underline">
                Retry
              </button>
            </div>
          )}
          ...
          <div className="bg-white rounded-lg border border-[#CFCBC8] relative z-0">
            {/* Table Header */}
            <div className="grid grid-cols-6 gap-8 px-8 py-4 bg-white border-b border-[#A9A4A0]">
              ...
              <div className="text-xs font-semibold text-[#3C3A39] uppercase tracking-wider">Role</div>
            ...
            {loading && (
              <div className="px-8 py-12 text-center">
                <div className="inline-block w-8 h-8 border-4 border-[#6D81C5] border-t-transparent rounded-full animate-spin mb-4"></div>
                <p className="text-sm text-gray-500">Loading users from server...</p>
              </div>
            )}
            ...
            {!loading && paginatedUsers.length > 0 ? (
              <div className="divide-y divide-gray-200 overflow-visible relative">
                {paginatedUsers.map((user) => (
                  <div
                    key={user.id}
                    className="grid grid-cols-6 gap-8 px-8 py-4 items-center hover:bg-gray-50 transition"
                  >
