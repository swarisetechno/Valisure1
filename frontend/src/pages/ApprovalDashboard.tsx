...
const ApprovalDashboard = () => {
  ...
  const dashboards = [
    ...
    { name: "Approval Dashboard", path: "#", current: true },
  ];
      ...
      <div className={`transition-all duration-300 ${sidebarOpen ? "lg:ml-64" : "lg:ml-24"}`}>
        ...
        <header className={`bg-[#1D2749] text-white h-20 flex items-center justify-between px-5 lg:px-8 fixed lg:absolute top-0 left-0 right-0 z-30 ${sidebarOpen ? "lg:left-64" : "lg:left-24"} transition-all duration-300`}>
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-end">
              <p className="text-sm font-semibold text-[#F7F7F7]">Welcome, Approver</p>
              <p className="text-xs text-gray-300">Approval Manager</p>
            </div>
            <div className="w-8 h-8 bg-gradient-to-br from-red-400 to-red-600 rounded-full border-2 border-[#FAC277]"></div>
          </div>
        </header>
        ...
        <main className="p-8 pt-24">
          <div className="bg-white rounded-xl shadow-lg p-12">
            <h1 className="text-4xl font-bold text-[#1D2749] mb-4">Approval Dashboard</h1>
            <p className="text-lg text-gray-600 mb-6">
              Welcome to the Approval Dashboard! This section is under development.
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gradient-to-br from-red-100 to-red-50 rounded-lg p-6 border border-[#3A4E92]">
                <h3 className="text-xl font-semibold text-[#1D2749] mb-2">Pending Approvals</h3>
                <p className="text-gray-600">Review pending requests</p>
              </div>
              
              <div className="bg-gradient-to-br from-orange-100 to-orange-50 rounded-lg p-6 border border-[#3A4E92]">
                <h3 className="text-xl font-semibold text-[#1D2749] mb-2">History</h3>
                <p className="text-gray-600">View approval history</p>
              </div>
              
              <div className="bg-gradient-to-br from-pink-100 to-pink-50 rounded-lg p-6 border border-[#3A4E92]">
                <h3 className="text-xl font-semibold text-[#1D2749] mb-2">Settings</h3>
                ...
                {dashboards.map((dashboard) => (
                  <li key={dashboard.name}>
                    <a
                      href={dashboard.path}
                      className={`${
                        dashboard.current ? "font-bold text-red-600" : "text-blue-600 hover:text-blue-800"
                      }`}
                    >
                      → {dashboard.name}
                    </a>
