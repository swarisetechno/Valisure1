...
const EditUser = () => {
  ...
  const handleConfirmDisable = () => {
    if (selectedUserForDisable) {
      const users = JSON.parse(localStorage.getItem("users") || "[]");
      const updatedUsers = users.map((u: User) =>
        u.id === selectedUserForDisable.id ? { ...u, status: "Inactive" } : u
      );
      localStorage.setItem("users", JSON.stringify(updatedUsers));
            ...
            <div className="bg-white rounded-lg border border-[#CFCBC8] overflow-hidden">
              ...
              {projects.length > 0 ? (
                projects.map((project, index) => (
                  <div key={index} className="grid grid-cols-5 gap-8 px-8 py-5 border-b border-[#CFCBC8] hover:bg-gray-50 items-center">
                    <div>
                      <p className="text-sm font-medium text-[#3C3A39]">{project.name}</p>
                      <p className="text-xs text-[#A9A4A0]">{project.id}</p>
                    </div>
                    <div>
                      <span className={`px-3 py-1 rounded text-xs font-semibold text-white ${
                        project.role === "Admin"
                          ? "bg-[#6D81C5]"
                          : project.role === "Reviewer-Approver"
                          ? "bg-[#FAC277]"
                          : "bg-[#91A1D4]"
                      }`}>
                        {project.role}
                    ...
                    <div>
                      <span className={`px-3 py-1 rounded text-xs font-semibold text-white ${
                        project.status === "Active" ? "bg-[#15803D]" : "bg-[#837F7C]"
                      }`}>
                        {project.status}
                      </span>
                    </div>
                    <div className="text-sm text-[#504539]">{project.lastAccess}</div>
