...
def _derive_name_parts(value: str) -> tuple[str, str]:
    ...
    last_name = parts[1].strip() if len(parts) > 1 and parts[1].strip() else first_name
...
def create_user_router(get_db):
    ...
    def login(
        ...
        try:
            login_value = req.username_or_email.strip()

            # Find user by email or username
            user = db.query(UserModel).filter(
                (UserModel.email == login_value) |
                (UserModel.username == login_value)
            ).first()
            ...
            try:
                password_matches = verify_password(req.password, user.password_hash)
            except Exception as password_error:
                print(f"[WARN] Password verification failed for user {user.id}: {password_error}")
                raise HTTPException(status_code=401, detail="Invalid credentials")
    ...
    def create_user(
        ...
        try:
            ...
            first_name, last_name = _derive_name_parts(req.username)
